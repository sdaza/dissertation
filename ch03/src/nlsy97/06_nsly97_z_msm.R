...
imp_z_absolute_mob = readRDS('output/data/nlsy97_z_absolute_mob_imputations.rds')

# transform imputation data

# relative
long_imp_z_relative_mob = data.table(
...
long_imp_z_relative_mob[, rev_health := factor(rev_health)]

# absolute
long_imp_z_absolute_mob = data.table(
...
long_imp_z_absolute_mob[, good_health :=
    ifelse(rev_health %in% c(4, 5), 1, 0)]
long_imp_z_absolute_mob[, rev_health := factor(rev_health)]
...
createModelTables(
    ...
    filename = "output/tables/nlsy97_unadjusted_z_models.tex",
    comment = comment,
    # groups = groups,
    observations = N
)
...
numerator = longText("
    ...
    lag_z_relative_mob) + as.factor(stime) + sweight
")
...
numerator = longText("
    ...
    baseline_log_income_adj + baseline_hhsize +
    baseline_imp_parent_employed + baseline_imp_parent_employed +
    baseline_rev_health + baseline_bmi + baseline_smoking + baseline_z_absolute_mob +
    baseline_z_gini + baseline_log_population +
    baseline_log_county_income + baseline_z_prop_black +
    lag_z_absolute_mob) + as.factor(stime) + sweight
")
...
denominator = longText("
    ...
    baseline_imp_parent_employed + baseline_imp_parent_employed +
    baseline_rev_health + baseline_smoking + baseline_bmi +
    baseline_z_absolute_mob + baseline_z_gini + baseline_log_population +
    baseline_log_county_income + baseline_z_prop_black +
    nmoves + lag_imp_parent_employed + lag_imp_parent_married + lag_hhsize +
    log_income_adj + lag_rev_health + lag_bmi + lag_smoking +
    lag_z_absolute_mob + lag_z_gini + lag_log_county_income +
    lag_log_population + lag_z_prop_black) + as.factor(stime) + sweight
")
...
numerator = longText("
    ...
    baseline_rev_health + baseline_smoking + baseline_bmi +
    baseline_z_relative_mob + baseline_z_gini + baseline_log_population +
    baseline_log_county_income + baseline_z_prop_black + lag_z_gini) +
    as.factor(stime) + sweight
")
...
predictors = longText(
    "average_z_gini + male + ethnicity +
     ...
     baseline_log_income_adj + baseline_hhsize +
     baseline_imp_parent_employed + baseline_imp_parent_married +
     baseline_z_relative_mob + baseline_log_population +
     baseline_log_county_income + baseline_z_prop_black
")
...
createModelTables(
    ...
    row_names = row_names,
    row_labels = row_labels,
    filename = "output/tables/nlsy97_adjusted_z_models.tex",
    comment = comment,
    # groups = groups,
    observations = N
)
...
tableWeights(list_weights,
             ...
             filename = "output/tables/nlsy97_ipt_weights_z.tex",
             tabcolsep = 10,
             arraystretch = 1)
...
# read imputed data
imp_zr_relative_mob = readRDS('output/data/nlsy97_zr_relative_mob_imputations.rds')
imp_zr_absolute_mob = readRDS('output/data/nlsy97_zr_absolute_mob_imputations.rds')

# relative
long_imp_zr_relative_mob = data.table(
...
long_imp_zr_relative_mob[, good_health :=
    ifelse(rev_health %in% c(4, 5), 1, 0)]
long_imp_zr_relative_mob[, rev_health := factor(rev_health)]

# absolute
long_imp_zr_absolute_mob = data.table(
...
long_imp_zr_absolute_mob[, good_health :=
    ifelse(rev_health %in% c(4, 5), 1, 0)]
long_imp_zr_absolute_mob[, rev_health := factor(rev_health)]
...
createModelTables(
    ...
    row_names = row_names,
    row_labels = row_labels,
    filename = "output/tables/nlsy97_unadjusted_zr_models.tex",
    comment = comment,
    # groups = groups,
    observations = N
)
...
numerator = longText("
    ...
    baseline_imp_parent_employed + baseline_imp_parent_employed +
    baseline_rev_health + baseline_bmi + baseline_smoking +
    baseline_relative_mob_resid + baseline_gini_resid + baseline_log_population +
    baseline_log_county_income + baseline_z_prop_black +
    lag_relative_mob_resid) + as.factor(stime) + sweight
")
...
denominator = longText("
    ...
    nmoves + lag_imp_parent_employed + lag_imp_parent_married + lag_hhsize +
    log_income_adj + lag_rev_health + lag_bmi + lag_smoking +
    lag_relative_mob_resid + lag_gini_resid + lag_log_county_income +
    lag_log_population + lag_z_prop_black) + as.factor(stime) + sweight
")
...
predictors = longText(
    "average_relative_mob_resid + male + ethnicity +
     ...
     baseline_log_income_adj + baseline_hhsize +
     baseline_imp_parent_employed + baseline_imp_parent_married +
     baseline_gini_resid + baseline_log_population +
     baseline_log_county_income + baseline_z_prop_black
")
...
numerator = longText("
    ...
    baseline_log_income_adj + baseline_hhsize +
    baseline_imp_parent_employed + baseline_imp_parent_employed +
    baseline_rev_health + baseline_bmi + baseline_smoking + baseline_absolute_mob_resid +
    baseline_gini_resid + baseline_log_population +
    baseline_log_county_income + baseline_z_prop_black +
    lag_absolute_mob_resid) + as.factor(stime) + sweight
")
...
denominator = longText("
    ...
    baseline_imp_parent_employed + baseline_imp_parent_employed +
    baseline_rev_health + baseline_smoking + baseline_bmi +
    baseline_absolute_mob_resid + baseline_gini_resid + baseline_log_population +
    baseline_log_county_income + baseline_z_prop_black +
    nmoves + lag_imp_parent_employed + lag_imp_parent_married + lag_hhsize +
    log_income_adj + lag_rev_health + lag_bmi + lag_smoking +
    lag_absolute_mob_resid + lag_z_gini + lag_log_county_income +
    lag_log_population + lag_z_prop_black) + as.factor(stime) + sweight
")
...
numerator = longText("
    ...
    baseline_rev_health + baseline_smoking + baseline_bmi +
    baseline_relative_mob_resid + baseline_gini_resid + baseline_log_population +
    baseline_log_county_income + baseline_z_prop_black + lag_gini_resid) +
    as.factor(stime) + sweight
")
...
predictors = longText(
    "average_gini_resid + male + ethnicity +
     ...
     baseline_log_income_adj + baseline_hhsize +
     baseline_imp_parent_employed + baseline_imp_parent_married +
     baseline_relative_mob_resid + baseline_log_population +
     baseline_log_county_income + baseline_z_prop_black
")
...
createModelTables(
    ...
    row_names = row_names,
    row_labels = row_labels,
    filename = "output/tables/nlsy97_adjusted_zr_models.tex",
    comment = comment,
    # groups = groups,
    observations = N
)
