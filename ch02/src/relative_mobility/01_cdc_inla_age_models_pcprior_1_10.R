...
# R < 00dissertation/ch02/src/01_cdc_inla_age_models_pcprior_1_10.R > 00dissertation/ch02/src/01_cdc_inla_age_models_pcprior_1_10.log  --no-save  &
...
#   coeff_labels=c('Relative Mobility', 'Gini'), exponential=TRUE)
# dev.off()

# savepdf('output/m1_random_effects__pcprior_1_10')
# plot_random_dist(m1, terms=c('mob_age', 'gini_age'),
#   term_labels=c('Relative Mobility', 'Gini'), exponential=FALSE)
# dev.off()
#
...
formula = deaths ~  1 + log_population + log_income + z_relative_mob  * z_gini +
        f(state_i, model = 'iid', hyper=pcprior) +
        f(county_i, model = 'iid', hyper=pcprior) +
        f(age, model = 'iid', hyper=pcprior) +
        f(id, model = 'iid', hyper=pcprior) +
